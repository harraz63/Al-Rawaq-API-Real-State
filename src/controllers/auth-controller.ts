...
const registerUser = async (req: Request, res: Response) => {
    try {
        ...
        const token = generateJWTToken(userCreated.id, "email-verification", userCreated.role)
        ...
        const isEmailSent = await sendEmailVerification(email, emailSubject, verificationLink, name);
    ...
    try {
        ...
        if (!user.isEmailVerified) {
            ...
            if (existingVerification && existingVerification.expiresAt > new Date) {
                ...
                const newToken = generateJWTToken(user.id, "email-verification", user.role);
                await Verification.create({
                    userId: user.id,
                    token: newToken,
                ...
                const emailSubject = "Verify your email";
                const isEmailSent = await sendEmailVerification(email, emailSubject, verificationLink, "بك");
...
const resetPasswordRequest = async (req: Request, res: Response) => {
    try {
        ...
        const resetPasswordToken = generateJWTToken(user.id, "reset-password", user.role);
        ...
        const isEmailSent = await sendEmailVerification(email, emailSubject, resetPasswordLink, user.name);
